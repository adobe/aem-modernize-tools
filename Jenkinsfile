...
pipeline {
  environment {
    ...
    PACKAGE_NAME = 'aem-modernize-tools-content'
  }
  agent any
  stages {
    stage('Build') {
...
static def stashJavadocs(script) {
  stashFiles(script, script.env.JAVADOC_STASH, 'bundles/aem-modernize-tools/target/apidocs', '**/*')
}

static def stashContentPackage(script) {
  stashFiles(script, script.env.PACKAGE_NAME, 'content/target', 'aem-modernize-tools-content-*.zip')
}
...
static def createGithubRelease(script, version) {
  ...
  def upload_url = data.upload_url.replaceAll(/\{\?name,label}/, '')
  return upload_url
}

static def uploadReleaseArtifacts(script, version, url) {
  script.unstash name: script.env.PACKAGE_NAME
  script.withCredentials([
      script.usernameColonPassword(credentialsId: script.env.GITHUB_TOKEN_CREDENTIALS_ID, variable: 'CREDENTIALS')
  ]) {
    ...
    script.sh """
      curl -u "\$CREDENTIALS" \
        ...
        --data-binary @${script.env.PACKAGE_NAME}-${version}.zip \
        ${url}?name=${script.env.PACKAGE_NAME}-${version}.zip
    """
  }
}
...
static def createVersionFile(script, version, content) {
  ...
  script.dir('_pages/releases') {
    ...
    script.dir(path) {
      script.writeFile file: "v${version}.md", text: fileContent
    }
  }
